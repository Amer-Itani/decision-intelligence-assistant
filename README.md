...
data/         10k prepared sample plus folders for raw/interim/processed data
...
On first backend startup, the container trains the deployable ML artifact from
the prepared 10k sample before Uvicorn becomes available. The first analyze call
after a fresh data change may also spend extra time populating Chroma; later
queries should be much faster.
...
The committed `data/sample/customer_support_sample.csv` is a representative
10k-row prepared sample generated from the full Kaggle `twcs.csv` file. The raw
full dataset is not committed because it is large; place it at
`data/raw/twcs.csv` when you want to rerun the notebook from scratch.
...
To regenerate the 10k-sample workflow, run
...
- The priority labels are weak labels generated by transparent rules, not human
  annotations. Metrics show how well models reproduce those rules.
- Some tickets have `unknown` brand metadata because the Twitter conversation
  links do not always expose a clear support-brand author.
- Chroma persistent mode is the primary retrieval path in Docker. The backend
  still includes a TF-IDF fallback for restricted local environments where
  Chroma cannot initialize.
